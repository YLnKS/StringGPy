...
/* ==== General spectral Matern ==== */
double sma_kernel(double x, double y, double *theta, unsigned int n){
    unsigned int i;
    double u, sig, l, freq, nu, res=0.0;
    u = x-y;
    ...
    for(i=0; i<n; i++){
        ...
        nu=theta[2+4*i];
        freq=theta[3+4*i];
        u = sqrt(2.0*nu)*fabs(x-y)/l;
        if(u==0.0){
           res += sig*sig; 
        }
        else{
            res += sig*sig/(tgamma(nu)*pow(2.0, nu-1.0))*pow(u, nu)*bessel_kv(nu, u)*cos(2.0*M_PI*u*freq);
        }
    }
...
double sma12_kernel(double x, double y, double *theta, unsigned int n){
    ...
    for(i=0; i<n; i++){
        ...
        freq=theta[2+3*i];
         ...
         /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0) & (l>0.0));
        res += sig*sig*exp(-fabs(u)/l)*cos(2.0*M_PI*u*freq);
    }
    return res;
}
...
double sma32_kernel(double x, double y, double *theta, unsigned int n){
    ...
    for(i=0; i<n; i++){
        ...
        freq=theta[2+3*i];
         ...
         /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0) & (l>0.0));
        res += sig*sig*(1+(sqrt(3.0)/l)*fabs(u))*exp(-(sqrt(3.0)/l)*fabs(u))*cos(2.0*M_PI*u*freq);
    }
    return res;
}
...
double sma52_kernel(double x, double y, double *theta, unsigned int n){
    ...
    for(i=0; i<n; i++){
        ...
        freq=theta[2+3*i];
         ...
         /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0) & (l>0.0));
        res += sig*sig*(1.0 + (sqrt(5.0)/l)*fabs(u) +
        	(5.0/(3.0*l*l))*u*u)*
        	exp(-(sqrt(5.0)/l)*fabs(u))*cos(2.0*M_PI*u*freq);
    }
    return res;
}
...
double srq_kernel(double x, double y, double *theta, unsigned int n){
    ...
    for(i=0; i<n; i++){
        ...
        /* Check that the hyper-parameters make sense */
		if ((sig<=0.0) | (freq<=0.0) | (l<=0.0) | (a<=0.0)){
			PyErr_SetString(PyExc_ValueError, "Hyper-paramaters must all be positive.");
		}
        res += sig*sig*pow(1.0+u*u/(2.0*a*l*l), -a)*
        cos(2.0*M_PI*u*freq);
    }
    return res;
}

/* ==== spectral Matern 1/2 - Matern 1/2 ==== */
double sma12_ma12_kernel(double x, double y, double *theta, unsigned int n){
    unsigned int i;
    double u, m, sig, l, l2, freq, res=0.0;
    u = x-y;
    m = 0.5*(x+y);
    ...
    for(i=0; i<n; i++){
        ...
        /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0) & (l>0.0) & (l2>0.0));
        res += sig*sig*exp(-fabs(u)/l)*cos(2.0*M_PI*u*freq)*exp(-fabs(m)/l2);
    }
    return res;
}

/* ==== spectral Matern 3/2 - Matern 1/2 ==== */
double sma32_ma12_kernel(double x, double y, double *theta, unsigned int n){
    ...
    for(i=0; i<n; i++){
        ...
        /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0) & (l>0.0) & (l2>0.0));
        res += sig*sig*(1+(sqrt(3.0)/l)*fabs(u))*exp(-(sqrt(3.0)/l)*fabs(u))*
        cos(2.0*M_PI*u*freq)*exp(-fabs(m)/l2);
    }
    return res;
}

/* ==== spectral Matern 5/2 - Matern 1/2 ==== */
...
/* ==== spectral RQ - Matern 1/2 ==== */
double srq_ma12_kernel(double x, double y, double *theta, unsigned int n){
    unsigned int i;
    double u, m, sig, l, l2, freq, res=0.0, a=0.0;
    u = x-y;
    m = 0.5*(x+y);
...
double sm_ma12_kernel(double x, double y, double *theta, unsigned int n){
    unsigned int i;
    double u, m, sig, l, l2, freq, res=0.0;
    u = x-y;
    m = 0.5*(x+y);
    ...
    for(i=0; i<n; i++){
        ...
        l2=theta[3+4*i];
         ...
         /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0) & (l>0.0) & (l2>0.0));
        res += sig*sig*exp(-2.0*M_PI*M_PI*u*u/(l*l))*cos(2.0*M_PI*u*freq)*
        	exp(-fabs(m)/l2);
    }
    return res;	
}
...
/* ==== Evaluate a Spectral Mixture kernel for a given input distance ==== */
double sm_kernel(double x, double y, double *theta, unsigned int n){
    unsigned int i;
    double u, sig, l, freq, res=0.0;
    u = x-y;
...
double dsm_kernel_dx(double x, double y, double *theta, unsigned int n){
    unsigned int i;
    double u, sig, l, freq, res=0.0;
    u = x-y;
    ...
    for(i=0; i<n; i++){
        ...
        /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0) & (l>0.0));
        res += sig*sig*exp(-2.0*M_PI*M_PI*u*u/(l*l))*(-2.0*M_PI*freq*sin(2.0*M_PI*u*freq)-(2.0*M_PI/l)*(2.0*M_PI/l)*u*cos(2.0*M_PI*u*freq));
    }
    return res;
}
...
double dsm_kernel_dy(double x, double y, double *theta, unsigned int n){
    return dsm_kernel_dx(y, x, theta, n);
}

/* ==== Evaluate the cross derivative of the SM kernel with respect to x and y. ==== */
double dsm_kernel_dxdy(double x, double y, double *theta, unsigned int n){
    unsigned int i;
    double u, sig, l, freq, res=0.0;
    u = x-y;
    ...
    for(i=0; i<n; i++){
        ...
        res += sig*sig*exp(-2.0*M_PI*M_PI*u*u/(l*l))*
            (-(2.0*M_PI/l)*(2.0*M_PI/l)*u*
                (-2.0*M_PI*freq*sin(2.0*M_PI*u*freq)-(2.0*M_PI/l)*(2.0*M_PI/l)*u*cos(2.0*M_PI*u*freq))
            + ((4.0*M_PI*M_PI*freq*freq + 4.0*M_PI/(l*l))*cos(2.0*M_PI*u*freq) 
                - 8.0*M_PI*M_PI*M_PI*freq*u*sin(2.0*M_PI*u*freq)/(l*l)));
    }
    return res;
}
...
double ss_kernel(double x, double y, double *theta, unsigned int n){
    ...
    for(i=0; i<n; i++){
        ...
        freq=theta[1+3*i];
         ...
         /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0));
        res += sig*sig*cos(2.0*M_PI*u*freq);
    }
    return res;
}
...
double dss_kernel_dx(double x, double y, double *theta, unsigned int n){
    ...
    for(i=0; i<n; i++){
        ...
        /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0));
		res += -2.0*M_PI*sig*sig*freq*sin(2.0*M_PI*u*freq);
    }
    return res;
}
...
double dss_kernel_dxdy(double x, double y, double *theta, unsigned int n){
    ...
    for(i=0; i<n; i++){
        ...
        /* Check that the hyper-parameters make sense */
        assert((sig>=0.0) & (freq>=0.0));
		res += pow(2.0*M_PI*sig*freq, 2)*cos(2.0*M_PI*u*freq);
    }
    return res;
}
...
/* ==== Evaluate the Matern kernel with any fraction order for given inputs. ==== */
double ma_kernel(double x, double y, double *theta){
    double u, nu=0.0, sig=0.0, l=0.0;
    u = sqrt(2.0*nu)*fabs(x-y)/l;
    ...
    return sig*sig/(tgamma(nu)*pow(2.0, nu-1.0))*pow(u, nu)*bessel_kv(nu, u);
}
...
double kernel(double x, double y, double *theta, unsigned int n, char *type){
    ...
    if(strcmp(type, "sma") == 0)
        return sma_kernel(x, y, theta, n);

	if(strcmp(type, "sma12_ma12") == 0)
		return sma12_ma12_kernel(x, y, theta, n);

	if(strcmp(type, "sma32_ma12") == 0)
		return sma32_ma12_kernel(x, y, theta, n);

	if(strcmp(type, "sma52_ma12") == 0)
		return sma52_ma12_kernel(x, y, theta, n);

	if(strcmp(type, "srq_ma12") == 0)
		return srq_ma12_kernel(x, y, theta, n);
	
	if(strcmp(type, "sm_ma12") == 0)
		return sm_ma12_kernel(x, y, theta, n);
...
double dkernel_dx(double x, double y, double *theta, unsigned int n, char *type){
	...
	if(strcmp(type, "sm") == 0)
		return dsm_kernel_dx(x, y, theta, n);
...
double dkernel_dxdy(double x, double y, double *theta, unsigned int n, char *type){
	...
	if(strcmp(type, "sm") == 0)
		return dsm_kernel_dxdy(x, y, theta, n);
...
void sm_cov(double **res, double *X1, unsigned int n_X1, double *X2, unsigned int n_X2, double *theta, unsigned int n_theta){
	cov(res, X1, n_X1, X2, n_X2, theta, n_theta, "sm");	
}
...
void sm_deriv_cov(double **res, double *X1, unsigned int n_X1, double *X2, unsigned int n_X2, double *theta, unsigned int n_theta){
	deriv_cov(res, X1, n_X1, X2, n_X2, theta, n_theta, "sm");
}
...
void string_sm_boundaries_deriv_cov(double **res, double *b_times, unsigned int n_b_times, double **thetas, unsigned int n_mixts){
	string_boundaries_deriv_cov(res, b_times, n_b_times, thetas, n_mixts, "sm");
}	
...
void string_deriv_cov(double **res, double *s_times, unsigned int n_s_times, double *b_times, unsigned int n_b_times, double **thetas, unsigned int n_mixts, char *type){
	...
	for(i=0; i<n_s_times; i++){
		if(((s_times[i]>b_times[j]) | (s_times[i]==b_times[0])) & (s_times[i]<=b_times[j+1])){
			counter +=1; // Still processing times in string j.
		}
		else{
			while(s_times[i]>b_times[j+1]){
				n_elts_per_string[j]=counter;
				counter=0;
				j+=1;
			}
			counter+=1;
...
void string_sm_deriv_cov(double **res, double *s_times, unsigned int n_s_times, double *b_times, unsigned int n_b_times, double **thetas, unsigned int n_mixts){
	string_deriv_cov(res, s_times, n_s_times, b_times, n_b_times, thetas, n_mixts, "sm");
}
