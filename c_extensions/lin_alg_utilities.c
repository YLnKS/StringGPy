...
*/
void duplicate_col(double **res, double *col, unsigned int n_row, unsigned int n_col){
	int i,j;
	for(i=0; i<n_row; i++){
		for(j=0; j<n_col; j++){
			res[i][j]=col[j];
...
void svd(double **cov, double **U, double *S, double **V, unsigned int n){
  ...
  if(n==2){
	...
	_cov=malloc(n*n*sizeof(double));
	_U=malloc(n*n*sizeof(double));
	_V=malloc(n*n*sizeof(double));
	work=malloc((n-1)*sizeof(double));

	// Copy 2D matrix into 1D column-major array
	to_column_major(cov, _cov, n, n);
	to_column_major(U, _U, n, n);
	to_column_major(V, _V, n, n);

	// Perform SVD using LAPACKE
	info = LAPACKE_dgesvd(LAPACK_COL_MAJOR, 'A', 'A', n, n, _cov, n, S, _U, n, _V, n, work);

	// Copy the result back into the 2D matrices 
	from_column_major(U, _U, n, n);
	from_column_major(V, _V, n, n);
	...
	if(info<0)
		PyErr_SetString(PyExc_ValueError, "SVD: illegal parameter value.");


	if(info>0)
		PyErr_SetString(PyExc_ValueError, "SVD: Did not converge.");
	}
}
...
double ill_cond_jit(double *eig_vals, int n){
	...
	// The maximum conditioning number allowed is 1e8
	oc=max_eig/min_eig;
	if(oc > 100000000.0){
		nc = fminf(oc, 100000000.0);
